...
function weights = decorrelation(x, random_state)
    ...
    labels = Z(:,end);
    data = Z(:,1:end-1);
    svm = fitcsvm(Z, labels, 'Standardize', true, 'KernelFunction', 'linear', 'BoxConstraint', 1, 'NumPrint', 0);
    supportVectors = svm.SupportVectors;  
    coefficients = svm.Alpha;  
    bias = svm.Bias;  
    probabilities = binaryClassification(Z, labels, supportVectors, coefficients, bias);
